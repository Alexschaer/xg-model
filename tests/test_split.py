...
def test_share_of_test_matches():
    mask = test_mask(MATCH_IDS, test_share=0.2)

    assert MATCH_IDS[mask].nunique() == 20


def test_same_seed_gives_same_split():
    assert test_mask(MATCH_IDS, seed=1).equals(test_mask(MATCH_IDS, seed=1))


def test_different_seed_gives_different_split():
    assert not test_mask(MATCH_IDS, seed=1).equals(test_mask(MATCH_IDS, seed=2))


def test_split_does_not_depend_on_row_order():
    shuffled = MATCH_IDS.sample(frac=1, random_state=0)

    original_test = set(MATCH_IDS[test_mask(MATCH_IDS)])
    shuffled_test = set(shuffled[test_mask(shuffled)])
